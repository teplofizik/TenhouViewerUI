...
namespace Mahjong.Helper
{
    ...
    class ShantenCalculator
    {
        ...
        private void TehaiCreate(Hand Hand)
        {
            ...
            {
                Mahjong.Sets.Set N = Hand.Naki[i];
                if (N.Type == Sets.SetType.NUKI) continue;

                for (int j = 0; j < N.Tiles.Length; j++)
                {
        ...
        private int CalcShantenNormal()
        {
            ...
            for (i = 0; i < Hand.Naki.Count; i++)
            {
                if (Hand.Naki[i].Type != Sets.SetType.NUKI) Mentsu++;
            }
